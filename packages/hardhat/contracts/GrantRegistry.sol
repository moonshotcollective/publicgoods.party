...
contract GrantRegistry {
     ...
     */
    function updateGrantMetaPtr(uint96 _id, MetaPtr calldata _metaPtr)
        external
    {
        Grant storage grant = grants[_id];
        require(msg.sender == grant.owner, "GrantRegistry: Not authorized");
     ...
     */
    function getGrants(uint96 _startId, uint96 _endId)
        public
        view
        returns (Grant[] memory)
    {
        require(
            _endId <= grantCount,
            "GrantRegistry: _endId must be <= grantCount"
        );
